...
class BaseCsvImporter:
    ...
    def import_from_directory(self,
        ...
        nodes = self._read_csv(base_dir / "nodes.csv",
                               self._spec.nodes)

        elements_1d = self._read_csv(base_dir / "elements_1d.csv",
                                     self._spec.elements_1d)

        elements_2d = self._read_csv(base_dir / "elements_2d.csv",
                                     self._spec.elements_2d)

        results = self._read_csv(base_dir / "results.csv",
                                 self._spec.results)
        ...
        return FEMModel.from_tables(nodes=nodes,
...
class ImporterRegistry:
    def __init__(self) -> None:
        self._registry = {
            CsvImportProfile.SAP2000: BaseCsvImporter(SAP2000_SPEC,
                                                      "sap2000"),
            CsvImportProfile.MIDAS_CIVIL: BaseCsvImporter(MIDAS_SPEC,
                                                          "midas_civil"),
            CsvImportProfile.SCIA_ENGINEER: BaseCsvImporter(SCIA_SPEC,
                                                            "scia_engineer"),
        }
