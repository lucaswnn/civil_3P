...
class FEMModel:
    ...
    def empty(cls) -> "FEMModel":
        return cls(
            ...
            elements_1d=pd.DataFrame(
                columns=["element_id", "node_i",
                         "node_j", "material_id", "section_id"]
            ),
            elements_2d=pd.DataFrame(
                columns=["element_id", "node_1", "node_2",
                         "node_3", "node_4", "material_id", "thickness"]
            ),
            properties=pd.DataFrame(
    ...
    @classmethod
    def from_tables(cls, **tables: pd.DataFrame) -> "FEMModel":
    ...
    def validate(self) -> None:
        ...
        required_2d_columns = {"element_id", "node_1", "node_2", "node_3"}
    ...
    def table_for(self, element_type: ElementType) -> pd.DataFrame:
    ...
    def property_map(self, entity_type: str) -> dict[tuple[str, str], Any]:
        filtered = self.properties[self.properties["entity_type"]
                                   == entity_type]
        return {
