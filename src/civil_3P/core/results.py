...
from civil_3P.core.enums import ElementType, ResultLocation, VisualizationMode
...
class ResultProcessor:
    ...
    def _process_1d(self, results: pd.DataFrame, selection: SelectionContext) -> pd.DataFrame:
        filtered = results[results["element_id"].isin(
            selection.selected_element_ids)].copy()
        aggregated = (
            filtered.groupby(["case_id", "result_name", "element_id"], as_index=False)[
                "value"]
            .max()
    ...
    def _process_2d(
        ...
        averaged = (
            filtered.groupby(["case_id", "result_name", "node_id"], as_index=False)[
                "value"]
            .mean()
