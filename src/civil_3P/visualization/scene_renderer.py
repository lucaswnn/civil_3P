...
from civil_3P.visualization.scene_viewer_config import (
    SceneViewerConfig
)
from civil_3P.standard.result_components import (
    ViewContentKind as vk
)
...
class SceneRenderer(QtInteractor):
    ...
    def load_result_scene(self, scene: Scene) -> None:
        ...
        if scene.result_view.kind == vk.NODE_POINTS:
            self._render_points(data)

        elif scene.result_view.kind == vk.ELEMENT_1D_PROFILE:
            self._render_lines(data)

        elif scene.result_view.kind == vk.ELEMENT_2D_UNIFORM:
            self._render_uniform_cells(
                data,
                scene.result_view.value_range,
            )

        elif scene.result_view.kind == vk.ELEMENT_2D_SHARED_NODES:
            self._render_shared_cells(
                data,
                scene.result_view.value_range,
            )

        elif scene.result_view.kind == vk.ELEMENT_2D_ISOLATED_NODES:
            self._render_isolated_cells(
                data,
                scene.result_view.value_range,
            )
            ...
            raise ValueError(
                "Unsupported view content kind: "
                f"{scene.result_view.kind}"
            )
    ...
    def _render_uniform_cells(
        ...
        if (
            not data.nodes.size
            or data.connection is None
            or data.element_type is None
        ):
            return
