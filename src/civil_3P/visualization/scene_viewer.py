...
class SceneViewer(QtInteractor):
    ...
    def _load_shells(self,
                     ...
                     cells: list[int],
                     celltypes: list[int],
                     element_ids: list[str] | None = None) -> None:
    ...
    def _render_element_1d_profile(self,
                                   scene: dict[str, Any],
                                   node_map: dict[str, int],
                                   points: np.ndarray,
                                   visualization: VisualizationData) -> None:
        ...
        for element_id, profile in visualization.element_station_values.items():
            ...
            start = points[node_map[bar[mc.ModelElement1DComponents.ELEMENT_1D_START_NODE]]]
    ...
    def _render_element_2d_uniform(self,
                                   scene: dict[str, Any],
                                   node_map: dict[str, int],
                                   points: np.ndarray,
                                   visualization: VisualizationData) -> None:
    ...
    def _render_element_2d_shared_nodes(self,
                                        scene: dict[str, Any],
                                        node_map: dict[str, int],
                                        points: np.ndarray,
                                        visualization: VisualizationData) -> None:
    ...
    def _render_element_2d_isolated_nodes(self,
        ...
        for element_id, node_values in visualization.element_node_values.items():
            ...
            values = np.array([node_values.get(node_id, np.nan)
                              for node_id in node_ids])

            face = pv.PolyData(local_points, faces=[len(node_ids), *range(len(node_ids))])
            face["value"] = values
